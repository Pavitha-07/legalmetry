/**
 * WebGL scanning-band background, ported near-verbatim from the reference
 * landing design (same `ogl` shader). Web-only: renders a raw <div>/<canvas>
 * DOM subtree rather than React Native primitives, since it is only ever
 * mounted from Landing.tsx, which only ever renders on Platform.OS==='web'.
 */
...
interface ScannerProps {
  ...
  glow?: number;
  scanDirection?: 'vertical' | 'horizontal' | 'diagonal' | string;
  colorSpread?: number;
  brightness?: number;
  contrast?: number;
  softness?: number;
...
export function Scanner({
  ...
  useEffect(() => {
    ...
    speed, sweepSpeed, sweepWidth, sweepFalloff, scale, frequency, ripple, bandDensity,
    lineSharpness, glow, colorSpread, brightness, contrast, softness, vignette, opacity,
    scanline, grain, grainIntensity, scanDirection, mouseInteraction, mouseRadius, mouseStrength,
    color1, color2, color3,
  ]);

  return <div ref={containerRef} className={`scanner-container ${className}`.trim()} style={style} />;
}
